...
class Pipeline:
    ...
    def run(self, multi_process=False):
        ...
        for repeat_index in range(1):
            ...
            np.save(self.dic_path["PATH_TO_MODEL"] + f'colight_allmap_{repeat_index}_{cnt_round}.npy', hiddens_allmap)
...
class CoLightAgent(Agent): 
    ...
    def build_network(
        ...
        #out:[batch,agent,action], att:[batch,layers,agent,head,neighbors]
        model=Model(inputs=In,outputs=[out,h])
...
if __name__ == '__main__':
    name = "test_for_hiddenstate_colight"
    config = {
        'name': name,
        "agent": "colight",
        "sumocfg_file": [
