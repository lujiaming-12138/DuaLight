...
class ConstructSample:
    ...
    def __init__(self, path_to_samples, cnt_round, cnt_gen, dic_traffic_env_conf):
        ...
        self.samples = []
        self.samples_all_intersection = [None]*self.dic_traffic_env_conf["maps_tl_nums"][cnt_gen]
    ...
    def load_data_for_system(self, folder):
        ...
        for i in range(self.dic_traffic_env_conf["maps_tl_nums"][self.cnt_gen]):
    ...
    def construct_state(self,features,time,i):
        ...
        state = self.logging_data_list_per_gen[i][int(time/15)]
    ...
    def construct_reward(self,rewards_components,time, i):
        ...
        rs = self.logging_data_list_per_gen[i][int((time + self.measure_time)/15)]
        assert time + self.measure_time == rs["time"]
        rs = self.get_reward_from_features(rs['state'])
        r_instant = self.cal_reward(rs, rewards_components)

        # average
        list_r = []
        for t in range(time, time + self.measure_time + 1, self.measure_time):
            #print("t is ", t)
            rs = self.logging_data_list_per_gen[i][int(t/15)]
            assert t == rs["time"]
            rs = self.get_reward_from_features(rs['state'])
    ...
    def judge_action(self,time,i):
        if self.logging_data_list_per_gen[i][int(time/15)]['action'] == -1:
            raise ValueError
        else:
            return self.logging_data_list_per_gen[i][int(time/15)]['action']
    ...
    def make_reward(self, folder, i):
        ...
        try:
            total_time = int(self.logging_data_list_per_gen[i][-1]['time'])
            # construct samples
            time_count = 0
    ...
    def make_reward_for_system(self):
        ...
        for folder in os.listdir(self.path_to_samples):
            if "generator_" + str(self.cnt_gen) != folder:
                continue
        ...
        for i in range(self.dic_traffic_env_conf["maps_tl_nums"][self.cnt_gen]):
            self.dump_sample(self.samples_all_intersection[i],"generator_{0}_inter_{1}".format(self.cnt_gen, i))
